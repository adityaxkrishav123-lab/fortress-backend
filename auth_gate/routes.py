...
async def register_volunteer(payload: VolunteerRegisterRequest, request: Request):
    ...
    await users_ref.set({
        ...
        "adhar_no": payload.adhar_no,
        "adhar_cert_url": payload.adhar_cert_url,
        "skill_cert_url": None,
        "pin_hash": None,
...
async def get_user_profile(request: Request):
    ...
    return UserProfileResponse(
        ...
        has_unread_updates=data.get("has_unread_updates", False),
...
async def update_user_profile(payload: UpdateProfileRequest, request: Request):
    """
    Update profile settings from Screen 15 and 19 (e.g., Theme, Language, About).
    Multi-collection sync: NGO fields sync to 'ngos' collection too.
    """
    db = _db(request)
    uid = request.state.uid
    ...
    # Get user to check role for NGO sync
    ...
    if user_doc.exists:
        ...
        # If user is NGO Admin and updating NGO fields, sync to 'ngos' collection
        if "ngo_email" in update_data:
            ngo_updates["email"] = update_data["ngo_email"]
        if "ngo_phone" in update_data:
            ngo_updates["contact"] = update_data["ngo_phone"]
            
        if ngo_id and ngo_updates and user_data.get("role") == "NGO_ADMIN":
            await db.collection("ngos").document(ngo_id).update(ngo_updates)
        
    await db.collection("users").document(uid).update(update_data)
    return {"status": "updated", "synced_ngo": bool(ngo_id and ngo_updates)}
